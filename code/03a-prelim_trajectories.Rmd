...
delta_annual_spp_rich <- ggplot(data = spp_rich %>% filter(treatments == "delta_annual" & exp_dates == "after"), 
             aes(x = date, y = spp_rich)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  geom_point(shape = annual_shape) +
  geom_smooth(color = annual_col, method = "lm") +
  labs(x = "Sampling date",
...
# ggsave(here::here("figures", "spp_rich-hq-2021-09-06.jpg"), plots_together, width = 10, height = 4, dpi = 150)
```

Some edits made on Monday 6 Sep 2021. Hello again! You are not going to die.
