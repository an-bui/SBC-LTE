...
# this is a mistake
fish_sessile <- biomass %>% 
  filter(group_mobility == "FISH_SESSILE") %>% 
  select(sp_code, scientific_name, common_name) %>% 
  unique()
...
for(i in 1:length(group_mobility)) {
  ...
  group_mobility_biomass_plot_list[i] <- paste(this_group)
  
  # make a separate object for the plot
  assign(paste(this_group), plot)
...
FISH_MOBILE
INVERT_MOBILE
FISH_SESSILE # again, this is a mistake
ALGAE_SESSILE
```
