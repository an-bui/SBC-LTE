...
```{r}
ggplot(df %>% filter(sp_code == "MAPY"), aes(x = treatment, y = dry_gm2)) +
...
ggplot(df %>% filter(sp_code != "MAPY" & group == "algae"), aes(x = treatment, y = dry_gm2)) +
  stat_summary(aes(group = treatment), fun.data = "mean_cl_boot", geom = "errorbar", width = 0.5, size = 1, na.rm = TRUE) +
  ...
  #       plot.title = element_text(size = 26), 
  #       legend.position = "none") +
  facet_wrap(~exp_dates+site)

ggplot(df %>% filter(group == "fish"), aes(x = treatment, y = dry_gm2)) +
  stat_summary(aes(group = treatment), fun.data = "mean_cl_boot", geom = "errorbar", width = 0.5, size = 1, na.rm = TRUE) +
  stat_summary(aes(col = treatment), fun = "mean", geom = "point", size = 10, na.rm = TRUE) +
  scale_color_manual(values = color_palette) +
  labs(x = "Treatment", y = expression(paste("Biomass (g dry/", "m"^2, ")", sep = "")),
...
ggplot(df %>% filter(mobility == "sessile" & group == "invert"), aes(x = treatment, y = dry_gm2)) +
  stat_summary(aes(group = treatment), fun.data = "mean_cl_boot", geom = "errorbar", width = 0.5, size = 1, na.rm = TRUE) +
  stat_summary(aes(col = treatment), fun = "mean", geom = "point", size = 10, na.rm = TRUE) +
  scale_color_manual(values = color_palette) +
  labs(x = "Treatment", y = expression(paste("Biomass (g dry/", "m"^2, ")", sep = "")),
