...
ggplot(delta_continual) +
  geom_line(aes(x = time_since_end, y = control, col = site), alpha = 0.5, size = 2.5, linetype = 4) +
...
kelp_during <- linear_model_kelp_during_glmmTMB %>% 
  tbl_regression(intercept = TRUE) %>% 
  bold_p(t = 0.05)
kelp_during
...
overall_ms <- ggplot() +
  ...
  geom_line(data = predicted_kelp_during_overall, aes(x = x, y = predicted), size = 2, alpha = 0.7) +
  geom_ribbon(data = predicted_kelp_during_overall, aes(x = x, ymax = conf.high, ymin = conf.low), alpha = 0.2) +
  scale_x_continuous(breaks = seq(-8, 6, by = 2), minor_breaks = NULL) +
  scale_y_continuous(breaks = seq(-1500, 1000, by = 1000), limits = c(-1800, 900)) +
  theme_bw() + 
  ...
  labs(x = "Time since end of removal (years)", 
       y = expression("\U0394"~kelp~biomass~"(treatment - control, "~dry~g/m^{"2"}~")"), 
       fill = "Site",
       shape = "Site")
...
ggsave(here::here("figures", paste("fig2_", todays_date, ".jpg", sep = "")), plot = overall_ms, height = 8, width = 12, dpi = 150)
```
