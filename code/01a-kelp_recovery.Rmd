...
ggplot(delta_continual) +
  ...
  geom_point(aes(x = time_since_end, y = control, shape = site, col = site), size = 1.5, alpha = 0.5, fill = "#FFFFFF") +
...
## c. all sites 2 year comparison

```{r}
sda_kelp_biomass <- ggplot(delta_continual %>% drop_na(comp_2yrs), aes(x = comp_2yrs, y = delta_continual, group = 1)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        panel.grid.minor = element_line(color = "white")) +
  labs(x = "Time period", y = "\U0394 kelp biomass (treatment - control)")
sda_kelp_biomass
```
...
delta_annual %>%
  ...
  stat_summary(aes(color = kelp_year, group = kelp_year), fun = mean, geom = "line", size = 2, alpha = 0.7) +
...
ggpredict(linear_model_kelp_lmer, terms = "time_since_end [6.4:6.9 by = 0.001]", type = "fixed")
...
ggpredict(linear_model_kelp_lmer, terms = "time_since_end [3:3.1 by = 0.001]", type = "random", condition = c(site = "napl"))
predicted_kelp_after_mohk <- ggpredict(linear_model_kelp_lmer, terms = ~ time_since_end, type = "random", condition = c(site = "mohk"))
ggpredict(linear_model_kelp_lmer, terms = "time_since_end [4.5:4.6 by = 0.01]", type = "random", condition = c(site = "mohk"))
predicted_kelp_after_carp <- ggpredict(linear_model_kelp_lmer, terms = ~ time_since_end, type = "random", condition = c(site = "carp"))
ggpredict(linear_model_kelp_lmer, terms = "time_since_end [3.4:3.5 by = 0.01]", type = "random", condition = c(site = "carp"))
...
# 7. potential manuscript plots

## a. annual removal kelp biomass

```{r}
ggsave(here::here("figures", paste("fig1_", todays_date, ".jpg", sep = "")), plot = biomass_by_season_annual, height = 6, width = 8, dpi = 150)
```
...
ggsave(here::here("figures", paste("fig2_", todays_date, ".jpg", sep = "")), plot = fig2, height = 8, width = 16, dpi = 150)
```
