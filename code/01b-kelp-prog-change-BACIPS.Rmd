...
# LINEAR MODEL SELECTED - Likelihood = 59.1%

aque_bacips_plot <- delta_continual %>% 
  filter(site == "aque" & exp_dates == "after") %>% 
  ggplot(aes(x = time_since_end, y = delta_continual)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  geom_line(color = aque_col, size = 2.5) +
  geom_point(shape = aque_shape, fill = "#FFFFFF", color = "#FFFFFF", size = 1) +
  geom_function(fun = function(x) -249.35+(15.8*x), size = 2) +
  labs(x = "Time since end", y = "\U0394 biomass",
...
napl_continual_bacips_results$model.test.df.asymptotic.Model

# SIGMOID MODEL SELECTED - Likelihood = 84.3%

napl_bacips_plot <- delta_continual %>% 
  filter(site == "napl" & exp_dates == "after") %>% 
  ggplot(aes(x = time_since_end, y = delta_continual)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  geom_line(color = napl_col, size = 2.5) +
  geom_point(shape = napl_shape, fill = "#FFFFFF", color = "#FFFFFF", size = 1) +
  geom_function(fun = function(x) (248.5 * (x/2.52)^17.89)/(1 + (x/2.52)^17.89) - 221.00, size = 2) +
  labs(x = "Time since end", y = "\U0394 biomass",
       title = "Naples (NAPL)") +
  theme_bw() + 
  theme(axis.title = element_text(size = 18),
...
# LINEAR MODEL SELECTED - Likelihood = 87.2%

mohk_bacips_plot <- delta_continual %>% 
  filter(site == "mohk" & exp_dates == "after") %>% 
  ggplot(aes(x = time_since_end, y = delta_continual)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  geom_line(color = mohk_col, size = 2.5) +
  geom_point(shape = mohk_shape, fill = "#FFFFFF", color = "#FFFFFF", size = 1) +
  geom_function(fun = function(x) -802.38+(70.47*x), size = 2) +
  labs(x = "Time since end", y = "\U0394 biomass",
       title = "Mohawk (MOHK)") +
  theme_bw() + 
  theme(axis.title = element_text(size = 18),
...
# STEP MODEL SELECTED - Likelihood = 51.3%
...
carp_bacips_plot <- delta_continual %>% 
  ...
  ggplot(aes(x = time_since_end, y = delta_continual)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  geom_line(color = carp_col, size = 2.5) +
  geom_point(shape = carp_shape, fill = "#FFFFFF", color = "#FFFFFF", size = 1) +
...
model_selection_summary_table <- cbind(
  ...
  as.data.frame() %>% 
  mutate(site = "Arroyo Quemado (AQUE)") %>% 
  rbind(
    cbind(
      model = list("Step", "Linear", "Asymptotic", "Sigmoid"),
