...
lm3 <- glmmTMB::glmmTMB(value ~ scale(time_since_end)*name + (1|site),
...
plot(ggpredict(lm3, terms = ~time_since_end*name), add.data = T) # Fast way to get a plot
...
lm5 <- glmmTMB::glmmTMB(value ~ scale(time_since_end)*name + (1|site),
                        data = df2, family = ziGamma(link = "log"), ziformula = ~.) 
summary(lm5)
model_checks <- DHARMa::simulateResiduals(lm5) # pretty good1
plot(model_checks)

plot(ggpredict(lm5, terms = ~time_since_end*name), add.data = T)
