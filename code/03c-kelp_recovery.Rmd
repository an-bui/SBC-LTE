...
delta_biomass <- kelp_biomass %>% 
  select(site, year, month, treatment, date, exp_dates, wm_gm2) %>% 
...
time.model.fxn.new <- function(site, treatment, season_choice) {
  ...
  kelp_biomass %>% 
    filter(season %in% season_choice) %>% 
    select(site, year, month, treatment, date, wm_gm2) %>% 
    ...
    mutate(exp_dates = fct_relevel(exp_dates, c("during", "after"))) %>% 
    arrange(exp_dates) %>% 
    # add a column for deltas (not used in function, but useful for later)
    mutate(delta = .[[7]] - .[[6]])
}
...
# STEP MODEL SELECTED - Likelihood = 64.4%
...
# LINEAR MODEL SELECTED - Likelihood = 64.9%
...
# LINEAR MODEL SELECTED - Likelihood = 49.1%

napl_continual_bacips_results <- biomass.pcbacips(napl_biomass_continual)

# SIGMOID MODEL SELECTED - Likelihood = 58.3%
...
# STEP MODEL SELECTED - Likelihood = 90.2%
...
# LINEAR MODEL SELECTED - Likelihood = 64.7%

mohk_continual_bacips_results <- biomass.pcbacips(mohk_biomass_continual)

# LINEAR MODEL SELECTED - Likelihood = 99.1%
...
# LINEAR MODEL SELECTED - Likelihood = 55.2%

carp_continual_bacips_results <- biomass.pcbacips(carp_biomass_continual)

# LINEAR MODEL SELECTED - Likelihood = 47.7%
...
Using delta biomass. Trying to get the data frame in order for annual removal first:  
...
Now for continual removal:

```{r}
delta_continual <- delta_biomass %>% 
  select(site, year, month, date, exp_dates, control, annual, delta_continual) %>% 
