...
dredge(linear_model) %>% 
  model.sel() %>% 
  gt()
linear_model_sel <- lm(delta_annual ~ time_since_end + site, data = annual_after, na.action = na.pass)
summary(linear_model_sel)
plot(simulateResiduals(linear_model_sel))
# ggeffects
predicted <- ggpredict(linear_model_sel, terms = ~ time_since_end + site + time_since_end*site, type = "fixed") %>% 
  rename("site" = group) %>% 
  left_join(., enframe(sites_full), by = c("site" = "name")) %>% 
  rename("site_full" = value)
# emmeans
grid <- ref_grid(linear_model_sel, cov.keep = c("time_since_end", "site"))
emm <- emmeans(grid, spec = c("time_since_end", "site"), level = 0.95) %>% 
  data.frame(.) %>% 
  left_join(., enframe(sites_full), by = c("site" = "name")) %>% 
  rename("site_full" = value)
...
ggplot() +
  ...
  geom_ribbon(data = predicted, aes(x = x, y = predicted, ymin = conf.low, ymax = conf.high, fill = site_full), alpha = 0.5) +
  geom_line(data = predicted, aes(x = x, y = predicted, fill = site_full)) +
  theme_bw() + 
  theme(legend.position = "none",
        axis.title = element_text(size = 14),
        plot.title = element_text(size = 18),
        ...
        strip.text = element_text(size = 10)) +
  labs(x = "Time since end of experiment", y = "\U0394 biomass (treatment - control)",
       title = "\U0394 annual - seasonal data", caption = "ggpredict")
...
ggplot() +
  ...
  geom_point(data = delta_annual, aes(x = time_since_end, y = delta_annual, color = exp_dates)) +
  scale_color_manual(values = c("during" = "grey", "after" = "black"), guide = "none") +
  geom_ribbon(data = emm, aes(x = time_since_end, y = emmean, ymin = lower.CL, ymax = upper.CL, fill = site_full), alpha = 0.5) +
  geom_line(data = emm, aes(x = time_since_end, y = emmean, col = site_full)) + 
  theme_bw() + 
  theme(legend.position = "none",
        axis.title = element_text(size = 14),
        plot.title = element_text(size = 18),
...
linear_model <- lm(mean_delta ~ mean_tse + site + mean_tse*site, data = annual_after_means, na.action = na.pass)
dredge(linear_model) %>% 
  model.sel() %>% 
  gt()
linear_model_sel <- lm(mean_delta ~ mean_tse + site + mean_tse*site, data = annual_after_means, na.action = na.pass)
summary(linear_model_sel)
plot(simulateResiduals(linear_model_sel))
# ggeffects
predicted <- ggpredict(linear_model_sel, terms = ~ mean_tse + site + mean_tse*site, type = "fixed") %>% 
  rename("site" = group) %>% 
  left_join(., enframe(sites_full), by = c("site" = "name")) %>% 
  rename("site_full" = value)
# emmeans
grid <- ref_grid(linear_model_sel, cov.keep = c("mean_tse", "site"))
emm <- emmeans(grid, spec = c("mean_tse", "site"), level = 0.95) %>% 
  data.frame(.) %>%
  left_join(., enframe(sites_full), by = c("site" = "name")) %>% 
  rename("site_full" = value)
...
ggplot() +
  ...
  geom_point(data = annual_means, aes(x = mean_tse, y = mean_delta, color = exp_dates)) +
...
ggplot() +
  ...
  geom_point(data = annual_means, aes(x = mean_tse, y = mean_delta, color = exp_dates)) +
  scale_color_manual(values = c("during" = "grey", "after" = "black"), guide = "none") +
  geom_ribbon(data = emm, aes(x = mean_tse, y = emmean, ymin = lower.CL, ymax = upper.CL, fill = site), alpha = 0.5) +
  geom_line(data = emm, aes(x = mean_tse, y = emmean, col = site)) +
  theme_bw() + 
  theme(legend.position = "none",
        axis.title = element_text(size = 14),
        plot.title = element_text(size = 18),
...
ggplot() +
  ...
  scale_color_manual(values = c("during" = "grey", "after" = "black"), guide = "none") +
  # predicted values
  geom_ribbon(data = predicted, aes(x = x, y = predicted, ymin = conf.low, ymax = conf.high, fill = quality), alpha = 0.5) +
  geom_line(data = predicted, aes(x = x, y = predicted)) +
  # theme
  theme_bw() + 
  theme(legend.position = "none",
...
ggplot() +
  ...
  geom_point(data = annual_means, aes(x = mean_tse, y = mean_delta, color = exp_dates)) +
  scale_color_manual(values = c("during" = "grey", "after" = "black"), guide = "none") +
  geom_ribbon(data = emm, aes(x = mean_tse, y = emmean, ymin = lower.CL, ymax = upper.CL, fill = quality), alpha = 0.5) +
