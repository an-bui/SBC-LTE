...
comparison_biomass_fxn <- function(site_choice) {
    ...
    filter(site == site_choice) %>% 
    mutate(bef_aft = case_when(
      exp_dates == "start" ~ "during",
      exp_dates == "during" ~ "during",
      exp_dates == "after" ~ "after"
    )) %>%
    mutate(bef_aft = fct_relevel(bef_aft, "during", "after")) %>% 
    ggplot(aes(x = bef_aft, y = total_wm_gm2, col = treatment, shape = treatment)) +
...
total_biomass_timeseries_fxn <- function(site_choice) {
  ...
  ggplot(data = total_biomass %>% filter(site == site_choice),
...
ivee_total_biomass_timeseries <- ggplot(data = total_biomass %>% filter(site == "IVEE"),
         aes(x = date, y = total_wm_gm2, color = treatment, shape = treatment)) +
    scale_x_date(limits = c(aque_start_date, as.Date("2022-02-02"))) +
    geom_hline(yintercept = 0, color = "grey") +
    annotate("rect", xmin = ivee_after_date, xmax = as.Date("2022-02-02"),
...
aque_biomass_timeseries <- delta_biomass_timeseries_fxn("AQUE")
napl_biomass_timeseries <- delta_biomass_timeseries_fxn("NAPL")
ivee_biomass_timeseries <- ggplot() +
    scale_x_date(limits = c(aque_start_date, as.Date("2022-02-02"))) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
    annotate("rect", xmin = ivee_after_date, xmax = as.Date("2022-02-02"),
  ...
  annotate("text", x = as.Date("2009-04-01"), y = delta_biomass %>% filter(site == "IVEE") %>% pull(delta_annual) %>% min()*1.1, label = "control > removal", size = 3) +
    ...
    labs(x = "Sampling date",
         y = "Total kelp biomass (wm/gm2)",
         title = "IVEE")
mohk_biomass_timeseries <- delta_biomass_timeseries_fxn("MOHK")
carp_biomass_timeseries <- delta_biomass_timeseries_fxn("CARP")


aque_biomass_timeseries
napl_biomass_timeseries
ivee_biomass_timeseries
mohk_biomass_timeseries
carp_biomass_timeseries
```
