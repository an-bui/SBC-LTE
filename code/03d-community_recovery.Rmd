...
algae_vs_kelp <- delta_algae_continual %>% 
  ...
  geom_smooth(method = "lm", se = FALSE, color = "#000000", size = 2) +
...
algae_time <- ggplot() +
  ...
  # new_scale("color") + 
  # overall
  geom_line(data = predicted_algae_after, aes(x = x, y = predicted), size = 2) +
  geom_ribbon(data = predicted_algae_after, aes(x = x, ymax = conf.high, ymin = conf.low), alpha = 0.4) +
  geom_line(data = predicted_algae_during, aes(x = x, y = predicted), size = 2) +
  geom_ribbon(data = predicted_algae_during, aes(x = x, ymax = conf.high, ymin = conf.low), alpha = 0.4) +
...
invert_time <- ggplot() +
  ...
  # new_scale("color") + 
  # overall
  geom_line(data = predicted_invert_after, aes(x = x, y = predicted), size = 2) +
  geom_ribbon(data = predicted_invert_after, aes(x = x, ymax = conf.high, ymin = conf.low), alpha = 0.4) +
  geom_line(data = predicted_invert_during, aes(x = x, y = predicted), size = 2) +
  geom_ribbon(data = predicted_invert_during, aes(x = x, ymax = conf.high, ymin = conf.low), alpha = 0.4) +
  scale_x_continuous(breaks = seq(-8, 5, by = 1), minor_breaks = NULL) +
  # scale_y_continuous(breaks = seq(-250, 750, by = 250), limits = c(-250, 750)) +
  theme_bw() + 
  theme(axis.title = element_text(size = 16),
