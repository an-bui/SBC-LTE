...
test_glmm <- glmer(delta_continual_algae ~ delta_continual + (1|year) + (1|site), 
                                data = delta_algae_continual, family = quasi(link = "log"))
...
algae_time <- linear_model_algae_lme %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
...
AICc(linear_model_algae_lme, linear_model_algae_lmer)
...
linear_model_algae_lme_during <- lme(delta_continual_algae ~ time_since_end, random = ~ 1|site, data = delta_algae_continual %>% filter(exp_dates == "during"), na.action = na.pass)
summary(linear_model_algae_lmer_during)
summary(linear_model_algae_lme_during)
algae_time_during <- linear_model_algae_lme_during %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
...
algae_time <- ggplot() +
  ...
  labs(x = "Time since end of removal", y = "\U0394 understory algal biomass")
...
```{r}
sda_algae_biomass <- ggplot(delta_algae_continual %>% drop_na(comp_2yrs), aes(x = comp_2yrs, y = delta_continual_algae, group = 1)) +
  ...
  labs(x = "Time period", y = "\U0394 algae biomass")
sda_algae_biomass
```
...
```{r}
ggplot(delta_algae_continual) +
  geom_line(aes(x = time_since_end, y = control_algae, col = site), alpha = 0.5, size = 2.5) +
  geom_point(aes(x = time_since_end, y = control_algae, shape = site, col = site), size = 1.5, alpha = 0.5, fill = "#FFFFFF") +
  ...
  labs(x = "Time since end of experiment", 
       y = expression(Algae~biomass~(dry~g/m^{"2"}))) +
...
epi_invert_time <- linear_model_invert_lme %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
...
AICc(linear_model_invert_lme, linear_model_invert_lmer)
...
linear_model_invert_lme_during <- lme(delta_continual_inverts ~ time_since_end, random = ~ 1|site, data = delta_invert_continual %>% filter(exp_dates == "during"), na.action = na.pass)
summary(linear_model_invert_lme_during)
MuMIn::r.squaredGLMM(linear_model_invert_lmer_during)

epi_invert_time_during <- linear_model_invert_lme_during %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
# ggeffects
...
invert_time <- ggplot() +
  ...
  theme(axis.title = element_text(size = 16),
        ...
        plot.margin = margin(0, 0, 0, 0)) +
  labs(x = "Time since end of removal", y = "\U0394 epilithic invert biomass")
```

### raw invert biomass
```{r}
raw_invert_time <- ggplot(delta_invert_continual) +
  geom_line(aes(x = time_since_end, y = control_inverts, col = site), alpha = 0.5, size = 2.5) +
  geom_point(aes(x = time_since_end, y = control_inverts, shape = site, col = site), size = 1.5, alpha = 0.5, fill = "#FFFFFF") +
...
```{r}
sda_invert_biomass <- ggplot(delta_invert_continual %>% drop_na(comp_2yrs), aes(x = comp_2yrs, y = delta_continual_inverts, group = 1)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  stat_summary(geom = "line", fun = "mean") +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2) +
  stat_summary(fun = mean, geom = "point", size = 6) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        plot.margin = margin(0, 0, 0, 0),
        panel.grid.minor = element_line(color = "white")) +
  labs(x = "Time period", y = "\U0394 epilithic invert biomass")
sda_invert_biomass
```
...
linear_model_clam_lme <- lme(delta_continual_clams ~ time_since_end, random = ~ 1|site, data = clam_continual_after, na.action = na.pass)
summary(linear_model_clam_lme)
# no effect of time since end

clam_time <- linear_model_clam_lme %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
...
linear_model_clam_lme_during <- lme(delta_continual_clams ~ time_since_end, random = ~ 1|site, data = delta_clam_continual %>% filter(exp_dates == "during"), na.action = na.pass)
summary(linear_model_clam_lme_during)
summary(linear_model_clam_lmer_during)
clam_time_during <- linear_model_clam_lme_during %>% 
  tbl_regression() %>% 
  bold_p(t = 0.05)
...
clam_time <- ggplot() +
  ...
  geom_point(data = delta_clam_continual, 
             aes(x = time_since_end, y = delta_continual_clams, fill = site, shape = site), size = 3, alpha = 0.9) +
  scale_shape_manual(values = shape_palette_site) +
  scale_fill_manual(values = color_palette_site) +
  # new_scale("color") + 
  # overall
  ...
  theme(axis.title = element_text(size = 16),
        ...
        legend.position = "none", 
        plot.margin = margin(0, 0, 0, 0)) +
  labs(x = "Time since end of removal", y = "\U0394 endolithic invert biomass")
...
raw_clam_time <- ggplot(delta_clam_continual) +
  ...
  theme(axis.title = element_text(size = 16),
        ...
        legend.position = "none", 
        plot.margin = margin(0, 0, 0, 0)) +
  labs(x = "Time since end of removal", y = expression(endolithic~invertebrate~biomass~(g/m^{"2"})))

tbl_merge(tbls = list(algae_time_during, epi_invert_time_during, clam_time_during),
          tab_spanner = c("Algae", "Epi", "Endo"))
```

### start-during-after

```{r}
sda_clam_biomass <- ggplot(delta_clam_continual %>% drop_na(comp_2yrs), aes(x = comp_2yrs, y = delta_continual_clams, group = 1)) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.3) +
  stat_summary(geom = "line", fun = "mean") +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2) +
  stat_summary(fun = mean, geom = "point", size = 6) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        plot.margin = margin(0, 0, 0, 0),
        panel.grid.minor = element_line(color = "white")) +
  labs(x = "Time period", y = "\U0394 endolithic invert biomass")
sda_clam_biomass
```
...
### raw clam biomass
```{r}
ggplot(delta_clam_continual) +
  geom_line(aes(x = time_since_end, y = control_clams, col = site), alpha = 0.5, size = 2.5) +
  geom_point(aes(x = time_since_end, y = control_clams, shape = site, col = site), size = 1.5, alpha = 0.5, fill = "#FFFFFF") +
...
ggsave(here::here("figures", paste("fig3_", todays_date, ".jpg", sep = "")), plot = fig3, height = 10, width = 14, dpi = 150)
```
