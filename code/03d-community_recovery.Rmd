...
sda_algae_biomass <- ggplot(algae_anova_df) +
  ...
  annotate("text", x = 1, y = 125, label = "a", size = 14) +
  annotate("text", x = 2, y = 125, label = "b", size = 14) +
  annotate("text", x = 3, y = 125, label = "a", size = 14) +
  # aesthetics
  theme_bw() +
  ...
  labs(x = "Time period", 
       y = expression(Predicted~"\U0394"~algae~biomass~(dry~g/m^{"2"})))
sda_algae_biomass
```
...
sda_invert_biomass <- ggplot(invert_anova_df) +
  ...
  geom_line(aes(x = x, y = predicted, group = 1)) +
  # add in post-hoc comparisons
  annotate("text", x = 1, y = 20, label = "a", size = 14) +
  annotate("text", x = 2, y = 20, label = "b", size = 14) +
  annotate("text", x = 3, y = 20, label = "b", size = 14) +
  # aesthetics
  theme_bw() +
  theme(axis.title = element_text(size = 18),
        plot.title = element_text(size = 18),
        ...
        panel.grid.minor = element_line(color = "white")) +
  labs(x = "Time period", 
       y = expression(Predicted~"\U0394"~epilithic~invert~biomass~(dry~g/m^{"2"})))
sda_invert_biomass
```
...
sda_clam_biomass <- ggplot(clam_anova_df) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        panel.grid.minor = element_line(color = "white")) +
  labs(x = "Time period", 
       y = expression(Predicted~"\U0394"~endolithic~invert~biomass~(dry~g/m^{"2"})))
sda_clam_biomass
```
...
group_summary_tables <- tbl_stack(
  tbls = list(algae_tables, epi_invert_tables, clam_tables),
  group_header = c("Algae", "Epilithic invertebrates", "Endolithic invertebrates"),
  quiet = TRUE) %>% 
  as_gt()
