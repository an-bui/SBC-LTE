...
```{r}
ggplot() +
  geom_point(data = algae_all_biomass %>% filter(site == "NAPL" & treatment == "CONTINUAL"), aes(x = date, y = total_biomass), color = under_col) +
  geom_line(data = algae_all_biomass %>% filter(site == "NAPL" & treatment == "CONTINUAL"), aes(x = date, y = total_biomass), color = under_col) +
  geom_point(data = algae_all_biomass %>% filter(site == "NAPL" & treatment == "CONTROL"), aes(x = date, y = total_biomass), color = under_col, alpha = 0.4) +
  geom_line(data = algae_all_biomass %>% filter(site == "NAPL" & treatment == "CONTROL"), aes(x = date, y = total_biomass), color = under_col, alpha = 0.4) +  
  geom_vline(aes(xintercept = napl_after_date)) +
  theme_bw()

ggplot() +
  geom_point(data = total_fronds %>% filter(site == "NAPL" & treatment == "CONTINUAL"), aes(x = date, y = total_fronds), color = kelp_col) +
  geom_line(data = total_fronds %>% filter(site == "NAPL" & treatment == "CONTINUAL"), aes(x = date, y = total_fronds), color = kelp_col) +
  geom_point(data = total_fronds %>% filter(site == "NAPL" & treatment == "CONTROL"), aes(x = date, y = total_fronds), color = kelp_col, alpha = 0.4) +
  geom_line(data = total_fronds %>% filter(site == "NAPL" & treatment == "CONTROL"), aes(x = date, y = total_fronds), color = kelp_col, alpha = 0.4) +
  geom_vline(aes(xintercept = napl_after_date)) +
  theme_bw()

test_fig_under <- ggplot(data = df_fig %>% filter(site == "NAPL"), 
                         aes(x = date, y = value, color = measurement, alpha = treatment)) +
  geom_point() +
  geom_line() +
  scale_alpha_manual(values = c("CONTINUAL" = 1, "CONTROL" = 0.4)) +
  scale_color_manual(values = c(total_fronds = kelp_col, total_biomass = under_col)) +
  geom_vline(aes(xintercept = napl_after_date)) +
  theme_minimal() +
  facet_wrap(~measurement, nrow = 2, scales = "free_y", strip.position = "right")
