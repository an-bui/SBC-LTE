...
ggplot() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  geom_point(data = delta_algae %>% filter(site == "AQUE" & delta_annual != "NA"), 
             aes(x = date, y = delta_annual, color = "Annual")) +
  geom_line(data = delta_algae %>% filter(site == "AQUE" & delta_annual != "NA"),
            aes(x = date, y = delta_annual, color = "Annual")) +
  geom_point(data = delta_algae %>% filter(site == "AQUE" & delta_continual != "NA"), 
             aes(x = date, y = delta_continual, color = "Continual")) +
  geom_line(data = delta_algae %>% filter(site == "AQUE" & delta_continual != "NA"),
            aes(x = date, y = delta_continual, color = "Continual")) +
  geom_vline(xintercept = aque_after_date, aes(color = "End"), show.legend = NA) +
  scale_color_manual(name = NULL,
                     breaks = c("Annual", "Continual", "End"),
                     values = c("Annual" = "green", "Continual" = "blue", "End" = "red")) +
  theme_bw() + 
  labs(title = "AQUE algal biomass", 
       x = "Sampling date",
       y = expression(Delta~"algal biomass"~"(control - removal)"))
...
ivee_algae_timeseries <- ggplot() +
  scale_x_date(limits = c(aque_start_date, as.Date("2020-06-01"))) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  geom_point(data = delta_fronds %>% filter(site == "IVEE" & delta_annual != "NA"), 
             aes(x = date, y = delta_annual), color = annual_col, shape = 19, size = 3) +
  geom_line(data = delta_fronds %>% filter(site == "IVEE" & delta_annual != "NA"),
            aes(x = date, y = delta_annual), color = annual_col) +
  geom_vline(xintercept = ivee_after_date, show.legend = NA) +
  theme_bw() + 
  labs(title = "IVEE",
       x = "Sampling date",
...
# ggsave(here::here("figures", "algae_biomass_ts.jpg"), plots_together, height = 9, width = 8, dpi = 200)
```
